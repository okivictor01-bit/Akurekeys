      ...
      if (url.pathname === '/// akurekeys worker v6 - rent escrow on real money
  ...
  async fetch(request, env) {
    try {
      ...
      if (url.pathname === '/health') {
        return Response.json({ ok: true, SUPABASE_URL: !!env.SUPABASE_URL, SUPABASE_ANON_KEY: !!env.SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY: !!env.SUPABASE_SERVICE_ROLE_KEY, PAYSTACK_SECRET_KEY: !!env.PAYSTACK_SECRET_KEY });
      }
      ...
      if (url.pathname === '/api/pay/verify' && request.method === 'POST') {
        ...
        const data = await v.json.json({ error: data.message }, { status: 400 });
