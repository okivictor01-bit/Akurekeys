  ...
  async fetch(request, env) {
    try {
      ...
      if (url.pathname === '/api/property/submit' && request.method === 'POST') {
        ...
        if (!b.title || !b.annual_rent_naira || !b.property_type || !b.street_address) {
        ...
        const ins = await fetch(env.SUPABASE_URL + '/rest/v1/properties', {
          ...
          body: JSON.stringify({
            ...
            annual_rent_naira: Number(b.annual_rent_naira),
            street_address: String(b.street_address),
            area_landmark: String(b.area_landmark || ''),
            bedrooms: b.bedrooms ? Number(b.bedrooms) : null,
            bathrooms: b.bathrooms ? Number(b.bathrooms) : null,
